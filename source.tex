...
\frame{
	...
	$$\ket{\theta} = \hat{H}_{\bc{1..n}} (\hat{U}_{f} \hat{H}_{\bc{1..n},\rc{n+1}} \ket{\bc{0..0}\rc{1}})_{\bc{1..n}}$$
}
\frame{
	\frametitle{Задача Дойча}
	$$\ket{\phi} = \hat{H}_{\bc{1..n},\rc{n+1}}\ket{\bc{0..0}\rc{1}} = 
...
\frame{
	\frametitle{Задача Дойча}
	$$\ket{\theta} = \gc{\hat{H}_{1..n}} \ket{\psi}_{1..n} = \frac{1}{2^{\frac{n}{2}}}\sum_{i=0}^{2^n-1}{(-1)^{f(i)}\gc{\hat{H}_{1..n}}\ket{i}}$$
	$$\gc{\hat{H}_{1..n}}\ket{i} = \frac{1}{2^{\frac{n}{2}}}(\bc{\ket{0..0}} + ...)$$
